...
namespace Watermelon
{
    public class LivesIndicator : MonoBehaviour
    {
        ...
        public void Init(LivesData data)
        {
            ...
            PremiumUIAssets.ApplyLifeIcon(transform);

            // Keep variable-length life timers inside their own area. The old
            // fixed 42px text overflowed left and rendered behind the heart.
            durationText.enableAutoSizing = true;
            durationText.fontSizeMin = 24f;
            durationText.fontSizeMax = 36f;
        ...
        private void SetTextSize(bool fullPanel)
        {
            if(fullPanel)
            {
                durationText.rectTransform.offsetMin = new Vector2(85, 0);
                durationText.rectTransform.offsetMax = new Vector2(-35, 0);
            }
            else
            {
                durationText.rectTransform.offsetMin = new Vector2(100, 0);
                durationText.rectTransform.offsetMax = new Vector2(-80, 0);
            }
        }
